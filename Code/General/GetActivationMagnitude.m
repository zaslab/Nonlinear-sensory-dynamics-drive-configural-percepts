function [all_act_diffs, unordered] = GetActivationMagnitude(...
...
for i = 1:size(unique_ids,1)
    ...
    point_to_plot = intersect(intersect(cond,step), neuron);
    
    isASHoff = (neuron_group(1)== 18 | neuron_group(1)== 3) & neuron_group(3)== 2;
    isASHoff = isASHoff | ((neuron_group(1)== 14 | neuron_group(1)== 22) & neuron_group(2)== 11 & neuron_group(3)== 2);
    isASHoff = isASHoff | ((neuron_group(1)== 14 | neuron_group(1)== 22) & neuron_group(2)== 10 & neuron_group(3)== 2);
    if isASHoff
        window = ASH_window;
    else 
        window = all_window;
    ...
    for worm = 1:length(post_act_mean)
        if (nanmean(post_act_mean) - nanmean(pre_act)) <(-0.1) & ~isASHoff
